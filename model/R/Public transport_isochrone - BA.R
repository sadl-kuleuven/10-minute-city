...
for(m in 1:nrow(piscines)){
  ...
  Id <- row_interest$Id
  Id <- as.data.frame(Id)
  Name <- row_interest$NOM.OFFICI
  Name_piscine <- as.data.frame(Name)
  
  #Create the buffer:
  buffer <- row_interest %>% st_buffer(dist = l)
  st_crs(buffer) = CRS("+init=epsg:31370")
  st_crs(stib_bus_times_unique) = CRS("+init=epsg:31370")
...
st_write(Isochrones_ALL, dsn = destination, layer = "BA 8", driver = "ESRI Shapefile", append = F, overwrite=T)
